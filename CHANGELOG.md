...
- Migrated project scaffolding from legacy AppVeyor + InvokeBuild + PSDepend +
  PSDeploy to Sampler-based build system.
- Migrated CI/CD from AppVeyor to Azure Pipelines with Build, Test, and Deploy
  stages.
- Restructured repository from `Datum.ProtectedData/public/` layout to
  Sampler `source/Public/` layout.
- Replaced `PSDepend.build.psd1` with `RequiredModules.psd1`.
- Replaced `Deploy.PSDeploy.ps1` with Sampler publish tasks in `build.yaml`.
- Replaced custom `.build.ps1` with standard Sampler `build.ps1`.
- Modernized module manifest with explicit `FunctionsToExport`, `PowerShellVersion`,
  `AliasesToExport`, `Tags`, `ProjectUri`, `LicenseUri`, and `IconUri`.
- Updated `tests/QA/module.tests.ps1` to Pester 5 syntax using
  `BeforeDiscovery`/`BeforeAll` blocks, `-ForEach` instead of `foreach`
  loops, and updated all legacy `Should` assertions to dash-parameter syntax.
- Migrated unit tests to Pester 5 syntax.
- Updated `.gitignore` for Sampler output directory.
- Updated `.vscode/settings.json` with full Sampler configuration.
- Added VSCode analyzer settings and launch configuration.

### Fixed

- Fix typo `'ByCertificae'` in `Unprotect-Datum.ps1` switch statement that
  prevented certificate-based decryption from working. Changed to
  `'ByCertificate'`.
